...
// render() - 
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void cMesh::render(cScene * pScene, cShaderInfo* pShaderInfo, bool wireframe)
{
	...
	mat4 gVP = pScene->m_Camera[pScene->m_cameraId].projectionMatrix * pScene->m_Camera[pScene->m_cameraId].viewMatrix;

	glUniformMatrix4fv(3, 1, GL_FALSE, glm::value_ptr(m_transform.m_ModelMatrix));
	glUniformMatrix4fv(4, 1, GL_FALSE, glm::value_ptr(gVP));

	vec3 camPos = pScene->m_Camera[pScene->m_cameraId].m_pos;

	//glUniform3fv(5, 1, glm::value_ptr(camPos));
	glUniform3fv(6, 1, glm::value_ptr(pScene->m_Light[pScene->m_lightId].m_pos));
	...
	// if wireframe is active find objects to wireframe and change shader ID

	if (wireframe && m_shdr != 4 && m_shdr !=0) {

		m_shdr = 3;
	}
	else { 
		m_shdr = m_shaderID; // if wireframe is not active dont change shader iD
	}
...
if (storedID  == 6) {
	...
	// if shader is not a tesselation shader then use GL_TRIANGLES
	if (m_shdr == 0|| m_shdr== 1 || m_shdr == 3|| m_shdr == 2|| m_shdr == 6)
	{
		glDrawArrays(GL_TRIANGLES, 0, vertexCount);
	}
	else if (m_shdr == 4|| m_shdr == 5) // if tesselated use GL_PATCHES
	{
